...
> Local-first, open-source AI assistant for Audit and Assurance. RAG over Australian auditing standards (**AUASB ASA**, with IAASB ISA as international reference) and a real ASX annual report (default: **Woolworths Group**, `WOW`), plus a journal-entry anomaly-detection engine that explains each flagged transaction with cited audit-standard references.
...
1. **Ask the auditor assistant.** Hybrid RAG (BM25 + dense + reciprocal-rank fusion) over AUASB ASA and the configured ASX annual report (default `WOW`). Every answer cites source, section, and page. Refuses when retrieval confidence is below threshold.
